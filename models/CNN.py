    ...
    def __init__(self,seq_length,n_features,n_convblock=2,filters=[16,32], num_neurons=[64,8]):
        ...
        for i in range(n_convblock):
            ...
            if i == 0:
                ...
                #self.architecture.add_module(f'maxpool_{i+1}',torch.nn.AvgPool1d(kernel_size = 2,stride = 2))
                #self.architecture.add_module(f'maxpool_{i+1}',torch.nn.LPPool1d(kernel_size = 2,stride = 2))
...
class ResNet(nn.Module):
    ...
    def __init__(self,block, num_blocks, num_classes=1):
        ...
        self.in_planes = CNN_Feautes
        self.conv1 = nn.Conv1d(15, CNN_Feautes, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn1 = nn.BatchNorm1d(CNN_Feautes)
        self.layer1 = self._make_layer(block, CNN_Feautes*2, num_blocks[0], stride=1)
        self.layer2 = self._make_layer(block, CNN_Feautes*4, num_blocks[1], stride=2)
        self.layer3 = self._make_layer(block, CNN_Feautes*8, num_blocks[2], stride=2)
        self.layer4 = self._make_layer(block, CNN_Feautes*16, num_blocks[3], stride=2)
        #self.linear = nn.Linear(2*512*block.expansion, num_classes)
        self.linear = nn.Linear(CNN_Feautes*16*3, num_classes)
    ...
    def _make_layer(self, block, planes, num_blocks, stride):
        ...
        for stride in strides:
            ...
            self.in_planes = planes*block.expansion # expansion = 1
...
class ResNetv2(nn.Module):
    ...
    def __init__(self,block, num_blocks, num_classes=1):
        ...
        self.conv1 = nn.Conv1d(15, CNN_Feautes, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn1 = nn.BatchNorm1d(CNN_Feautes)
        self.layer1 = self._make_layer(block, CNN_Feautes, num_blocks[0], stride=1)
        self.layer2 = self._make_layer(block, CNN_Feautes//2, num_blocks[1], stride=2)
        self.layer3 = self._make_layer(block, CNN_Feautes//4, num_blocks[2], stride=2)
        self.layer4 = self._make_layer(block, CNN_Feautes//8, num_blocks[3], stride=2)
        #self.linear = nn.Linear(2*512*block.expansion, num_classes)
        self.linear = nn.Linear(CNN_Feautes//8*3, num_classes)
